...
const PATTERNS: Array<{ name: string; pattern: RegExp; replacement: string }> =
  [
    ...
    },
    {
      name: "api_key",
      pattern:
        /\b(?:api[_-]?key|apikey|access[_-]?token|secret[_-]?key|auth[_-]?token|bearer)\s*[:=]\s*['"]?[A-Za-z0-9_\-./+=]{16,}['"]?/gi,
    ...
    },
    {
      name: "hex_secret",
      pattern:
        /\b(?:secret|token|key|hash)\s*[:=]\s*['"]?[0-9a-fA-F]{32,}['"]?/gi,
    ...
    },
    {
      ...
      pattern:
        /\b(?:(?:dev|staging|prod|internal|corp|vpn|admin|db|api|app|web|mail|git|ci|cd)\d*\.)[a-z0-9\-]+\.(?:internal|local|corp|intra|private|lan)\b/gi,
      replacement: "[REDACTED_HOSTNAME]",
    },
    {
      name: "internal_url",
      pattern:
        /https?:\/\/(?:(?:10|172\.(?:1[6-9]|2\d|3[01])|192\.168)\.\d+\.\d+|localhost)(?::\d+)?[^\s"']*/g,
...
export function redactReport(text: string): RedactionResult {
  ...
  for (const companyPattern of COMPANY_INDICATORS) {
    ...
    redactedText = redactedText.replace(regex, (match) => {
      ...
      return match.replace(
        /([A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2})\s+(Inc|Corp|LLC|Ltd|Co|Company|Group|Technologies|Solutions|Systems|Labs|Security|Software|Platform)/,
        "[REDACTED_COMPANY] $2",
      );
    });
    if (count > 0) {
  ...
  const totalRedactions = Object.values(categories).reduce(
    (sum, n) => sum + n,
    0,
  );
