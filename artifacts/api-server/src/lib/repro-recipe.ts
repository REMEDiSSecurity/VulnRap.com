...
const HTTP_REQUEST_RE =
  /(?:GET|POST|PUT|DELETE|PATCH|OPTIONS|HEAD)\s+(?:https?:\/\/[^\s]+|\/[^\s]*)/gi;
...
const HARDWARE_PATTERNS: HardwarePattern[] = [
  {
    ...
    vendor: "Intel",
    pattern:
      /\b(?:intel\s+)?(?:core\s+i[3579]|xeon|celeron|pentium|atom|alder\s*lake|raptor\s*lake|sapphire\s*rapids|ice\s*lake|tiger\s*lake|meteor\s*lake)\b/i,
    modelExtract:
      /\b((?:Core\s+i[3579]|Xeon|Celeron|Pentium|Atom)[\w\s-]{0,30})/i,
    productUrlTemplate:
  ...
  },
  {
    ...
    vendor: "AMD",
    pattern:
      /\b(?:amd\s+)?(?:ryzen|epyc|threadripper|athlon|opteron|zen\s*[234]?)\b/i,
    modelExtract:
      /\b((?:Ryzen|EPYC|Threadripper|Athlon|Opteron)[\w\s-]{0,30})/i,
  ...
  },
  {
    ...
    vendor: "ARM",
    pattern:
      /\b(?:arm\s+)?(?:cortex[-\s]?[amr]\d+|aarch64|armv[78]|neoverse|arm64|apple\s+m[1234])\b/i,
    modelExtract:
      /\b((?:Cortex[-\s]?[AMR]\d+|Neoverse[\w\s-]{0,20}|Apple\s+M\d))/i,
    emulationOptions: [
  ...
  },
  {
    ...
    vendor: "Dell",
    pattern:
      /\b(?:dell\s+)?(?:poweredge|idrac|dell\s+emc|openmanage|wyse|optiplex|latitude|precision|vostro|inspiron)\b/i,
    modelExtract:
      /\b((?:PowerEdge|iDRAC|OptiPlex|Latitude|Precision|Wyse)[\w\s-]{0,30})/i,
    productUrlTemplate:
      "https://www.dell.com/support/home/en-us/product-support/product/",
    emulationOptions: [
  ...
  },
  {
    ...
    vendor: "HPE",
    pattern:
      /\b(?:hpe?\s+)?(?:proliant|ilo|superdome|moonshot|edgeline|simplivity|nimble|primera)\b/i,
    modelExtract:
      /\b((?:ProLiant|iLO|Superdome|Moonshot|Edgeline)[\w\s-]{0,30})/i,
  ...
  },
  {
    ...
    vendor: "Lenovo",
    pattern:
      /\b(?:lenovo\s+)?(?:thinkserver|thinksystem|xclarity|thinkagile|thinkedge)\b/i,
    modelExtract:
      /\b((?:ThinkServer|ThinkSystem|XClarity|ThinkAgile|ThinkEdge)[\w\s-]{0,30})/i,
    productUrlTemplate: "https://support.lenovo.com/us/en/search?query=",
    emulationOptions: [
      "IPMI emulation via ipmi_sim",
  ...
  },
  {
    ...
    vendor: "Cisco",
    pattern:
      /\b(?:cisco\s+)?(?:ios[-\s]?xe?|nx-os|asa|firepower|catalyst|nexus|meraki|webex|aironet|csr\s*1000|isr\s*[1-4])\b/i,
    modelExtract:
      /\b((?:Catalyst|Nexus|ASA|Firepower|Meraki|Aironet|CSR|ISR)[\w\s-]{0,30})/i,
  ...
  },
  {
    ...
    vendor: "Juniper",
    pattern:
      /\b(?:juniper\s+)?(?:junos|srx|mx\s*\d+|ex\s*\d+|qfx|contrail|mist)\b/i,
  ...
  },
  {
    ...
    vendor: "Fortinet",
    pattern:
      /\b(?:fortinet\s+)?(?:fortigate|fortios|fortimanager|fortianalyzer|fortiweb|fortimail|fortisandbox)\b/i,
    modelExtract:
      /\b((?:FortiGate|FortiOS|FortiManager|FortiAnalyzer|FortiWeb)[\w\s-]{0,30})/i,
  ...
  },
  {
    ...
    vendor: "Palo Alto",
    pattern:
      /\b(?:palo\s*alto\s+)?(?:pan-os|pa-\d+|panorama|globalprotect|prisma|cortex\s+xdr|wildfire)\b/i,
    modelExtract:
      /\b((?:PA-\d+|PAN-OS|Panorama|GlobalProtect|Prisma)[\w\s-]{0,30})/i,
  ...
  },
  {
    ...
    vendor: "Generic IoT",
    pattern:
      /\b(?:mqtt|coap|zigbee|z-wave|bluetooth\s*le|ble|lorawan|modbus|bacnet|opc[-\s]?ua|scada|plc|hmi|dcs|rtu)\b/i,
    emulationOptions: [
  ...
  },
  {
    ...
    vendor: "Raspberry Pi",
    pattern:
      /\b(?:raspberry\s*pi|rpi|raspbian|raspberry\s*pi\s*(?:zero|pico|[2345]))\b/i,
  ...
  },
  {
    ...
    vendor: "Firmware",
    pattern:
      /\b(?:firmware|uefi|bios|bootloader|u-boot|grub|secure\s*boot|tpm|trusted\s*platform|sgx|trustzone|tee)\b/i,
    emulationOptions: [
  ...
  },
  {
    ...
    vendor: "NAS/SAN",
    pattern:
      /\b(?:synology|qnap|netapp|truenas|freenas|iscsi|fibre\s*channel|san\s+switch|nas\s+device|drobo|buffalo\s+nas)\b/i,
  ...
  },
  {
    ...
    vendor: "NVIDIA",
    pattern:
      /\b(?:nvidia\s+)?(?:cuda|geforce|rtx\s*\d+|gtx\s*\d+|tesla|a100|h100|quadro|jetson|dgx)\b/i,
    modelExtract:
      /\b((?:RTX|GTX|Tesla|A100|H100|Quadro|Jetson|DGX)[\w\s-]{0,20})/i,
...
const LANG_SIGNALS: Array<{ lang: string; pm: string; patterns: RegExp[] }> = [
  ...
  },
  {
    lang: "PHP",
    pm: "composer",
    patterns: [/\bphp\b/i, /\bcomposer\b/i, /\b<\?php\b/i, /\blaravel\b/i],
  },
  {
    lang: "Ruby",
    pm: "gem",
    patterns: [/\bruby\b/i, /\bgem\b/i, /\brails\b/i, /\bGemfile\b/i],
  },
  {
    lang: "Go",
    pm: "go",
    patterns: [/\bgo(?:lang)?\b/i, /\bgo\.mod\b/i, /\bfunc\s+\w+\s*\(/i],
  },
  {
    lang: "Rust",
    pm: "cargo",
    patterns: [/\brust\b/i, /\bcargo\b/i, /\bCargo\.toml\b/i],
  },
  {
    lang: "C/C++",
    pm: "make",
    patterns: [
      /\b(?:gcc|g\+\+|clang|make|cmake)\b/i,
      /\b#include\s*</i,
      /\bMakefile\b/i,
...
function extractPocCommands(
  text: string,
): { script: string; language: string } | null {
  ...
  if (genericBlocks.length > 0) {
    ...
    return {
      script: genericBlocks.map((b) => b.code).join("\n\n"),
      language: first.lang,
    };
  }
...
function buildSetupCommands(
  ...
  if (repoUrl) {
    ...
    if (version) {
      cmds.push(
        `git checkout v${version} 2>/dev/null || git checkout ${version} 2>/dev/null || echo "Version tag not found, using latest"`,
      );
    }
  }
...
function buildDockerfile(
  ...
  if (
    ...
    lines.push(
      `RUN apt-get update && apt-get install -y ${installPkgs.join(" ")} && rm -rf /var/lib/apt/lists/*`,
    );
  }
  ...
  if (repoUrl) {
    ...
    if (version) {
      lines.push(
        `RUN git checkout v${version} 2>/dev/null || git checkout ${version} || true`,
      );
    }
  } else {
    lines.push("COPY . .");
...
export function generateReproRecipe(
  ...
  const setupCommands = buildSetupCommands(
    primaryRepo,
    primaryVersion,
    langInfo,
    packageInstalls,
  ...
  if (llmTriageGuidance?.environment) {
    for (const env of llmTriageGuidance.environment) {
      ...
      if (
        envLower.includes("install") ||
        envLower.includes("setup") ||
        envLower.includes("configure")
      ) {
        const asCmd = env
          .replace(/^(?:install|set up|configure)\s*/i, "")
          .trim();
        if (
          asCmd.length > 3 &&
          !setupCommands.some((c) =>
            c.toLowerCase().includes(asCmd.toLowerCase()),
          )
        ) {
  ...
  if (verification) {
    ...
    const notFound = verification.checks.filter(
      (c) => c.result === "not_found",
    );

    if (verified.length > 0) {
      notes.push(
        `${verified.length} reference(s) verified against live sources (${verified.map((c) => c.target).join(", ")})`,
      );
    }
    if (notFound.length > 0) {
      notes.push(
        `WARNING: ${notFound.length} reference(s) not found — ${notFound.map((c) => `${c.target}: ${c.detail || "not found"}`).join("; ")}`,
      );
    }
  }
  ...
  if (!primaryRepo) {
    notes.push(
      "No repository URL detected in report — you may need to identify the target project manually.",
    );
  }
  if (!primaryVersion) {
    notes.push(
      "No specific version detected — check with the reporter which version was tested.",
    );
  }
  if (!poc) {
    notes.push(
      "No runnable PoC commands extracted — the report may describe the issue without executable steps.",
    );
  }
  ...
  for (const hw of hardwareComponents) {
    if (hw.productUrl) {
      notes.push(
        `${hw.vendor} ${hw.model || hw.type}: product info at ${hw.productUrl}`,
      );
    }
    if (hw.emulationOptions.length > 0) {
      notes.push(
        `Emulation for ${hw.vendor} ${hw.model || hw.type}: ${hw.emulationOptions[0]}`,
      );
    }
    if (hw.type === "iot" || hw.type === "embedded") {
      notes.push(
        "Hardware-dependent vulnerability — consider using firmware emulation (Firmadyne/QEMU) if physical device is unavailable.",
      );
    }
    if (hw.type === "network") {
      notes.push(
        "Network device vulnerability — consider GNS3/EVE-NG lab or vendor's virtual appliance for testing.",
      );
    }
  }
