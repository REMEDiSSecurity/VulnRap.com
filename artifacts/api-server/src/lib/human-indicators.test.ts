...
describe("detectHumanIndicators", () => {
  it("detects contractions as human signal", () => {
    const text =
      "I don't think this is right. It won't work because the input isn't validated. There aren't any checks on the boundary. The server doesn't enforce the limit correctly.";
  ...
  it("detects informal abbreviations", () => {
    const text =
      "Found this issue while testing tbh. The endpoint iirc was /api/users. Basically fwiw the auth is broken and anyone can access admin routes.";
    const result = detectHumanIndicators(text);
    const types = result.indicators.map((i) => i.type);
  ...
  it("detects commit/PR references", () => {
    const text =
      "This was introduced in commit a1b2c3d4e5f6g and partially fixed in PR #42. The regression appeared after merge request #15 was applied to the main branch.";
    const result = detectHumanIndicators(text);
    const types = result.indicators.map((i) => i.type);
  ...
  it("detects patched version references", () => {
    const text =
      "This vulnerability was fixed in version 2.3.1 but the fix was incomplete. The issue was properly patched in v2.4.0 with additional boundary checks added to the input parser.";
    const result = detectHumanIndicators(text);
    const types = result.indicators.map((i) => i.type);
  ...
  it("detects named researcher attribution", () => {
    const text =
      "This issue was reported by Alice Johnson who discovered the flaw during a routine code audit. The vulnerability affects the core authentication module and requires immediate attention.";
    const result = detectHumanIndicators(text);
    const types = result.indicators.map((i) => i.type);
  ...
  it("does NOT flag pleasantries if present", () => {
    const text =
      "Dear Security Team, I hope this finds you well. I would like to report a vulnerability in your application. Thank you for your time. Best regards, Researcher. " +
      "x ".repeat(50);
    const result = detectHumanIndicators(text);
    const types = result.indicators.map((i) => i.type);
  ...
  it("returns empty indicators for generic AI-like text", () => {
    const text =
      "It is important to note that this vulnerability represents a significant security risk. In the realm of cybersecurity, proactive measures must be taken to ensure robust security. The implications of this finding are multifaceted and require a holistic approach to remediation.";
    const result = detectHumanIndicators(text);
  ...
  it("detects terse writing style", () => {
    const text =
      "Buffer overflow. Stack smash. No ASLR. No canary. Trivial RCE. Heap spray works. Got shell in 3 tries. Patch the bounds check. Test attached. See crash log. Affects v2 and v3. Fix is one-liner. " +
      "Short line. ".repeat(10);
    const result = detectHumanIndicators(text);
    const types = result.indicators.map((i) => i.type);
