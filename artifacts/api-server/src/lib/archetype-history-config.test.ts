// Task #99 — unit tests for the persisted compaction-window config.
...
describe("archetype-history-config", () => {
  let tmpDir: string;
  let prevCfgPath: string | undefined;
  let prevEnvDays: string | undefined;

  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "archetype-history-cfg-"));
    prevCfgPath = process.env.ARCHETYPE_HISTORY_CONFIG_PATH;
    prevEnvDays = process.env.ARCHETYPE_HISTORY_COMPACT_DAYS;
    process.env.ARCHETYPE_HISTORY_CONFIG_PATH = path.join(tmpDir, "cfg.json");
  ...
  it("lets the env var override any persisted value", async () => {
    ...
    process.env.ARCHETYPE_HISTORY_COMPACT_DAYS = "14";
    const setting = getCompactAfterDaysSetting();
  ...
  it("ignores out-of-range persisted files (treats as no setting)", async () => {
    const cfgPath = process.env.ARCHETYPE_HISTORY_CONFIG_PATH!;
