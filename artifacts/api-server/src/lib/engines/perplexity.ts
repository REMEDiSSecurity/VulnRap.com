...
export function computePerplexity(
  text: string,
  codeBlocks: { lang: string | null; body: string }[] = [],
): PerplexityResult {
  const tokens = (text.toLowerCase().match(/\b[a-z][a-z']+\b/g) ?? []).slice(
    0,
    8000,
  );
  if (tokens.length < 20) {
  ...
  const entropyAiness = Math.max(
    0,
    Math.min(100, ((10.6 - entropy) / (10.6 - 9.0)) * 100),
  );
  const fwAiness = Math.max(
    0,
    Math.min(100, ((functionWordRate - 4) / (12 - 4)) * 100),
  );
  const syntaxPenalty = (1 - syntaxValidityScore) * 40; // bad code blocks are AI-tell

  const combinedScore = Math.max(
    0,
    Math.min(100, entropyAiness * 0.5 + fwAiness * 0.4 + syntaxPenalty * 0.1),
  );

  return {
    bigramEntropy: entropy,
    functionWordRate,
    syntaxValidityScore,
    combinedScore,
  };
...
function computeSyntaxValidity(
  codeBlocks: { lang: string | null; body: string }[],
): number {
  ...
  for (const cb of codeBlocks) {
    ...
    const balance =
      opens === 0 && closes === 0
        ? 1
        : 1 -
          Math.min(1, Math.abs(opens - closes) / Math.max(opens, closes, 1));
    // Has identifiers / not just prose
    const hasCodeShape =
      /[;{}=]|\bdef\b|\bfunction\b|\bclass\b|\breturn\b|\bif\b|\bfor\b|\bimport\b|\b#include\b|\$\s/.test(
        body,
      );
