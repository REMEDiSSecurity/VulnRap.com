...
const BENCH: Bench[] = [
  { id: 64, name: "Generic XSS",            type: "slop",  e1: 30, e2: 7,  e3: 38, observedComposite: 22 },
  { id: 67, name: "Generic SQLi",           type: "slop",  e1: 34, e2: 23, e3: 47, observedComposite: 35 },
  { id: 68, name: "Fake UAF",               type: "slop",  e1: 38, e2: 26, e3: 47, observedComposite: 36 },
  { id: 70, name: "Fake HTTP/2 DoS",        type: "slop",  e1: 33, e2: 22, e3: 68, observedComposite: 43 },
  { id: 72, name: "Auth Bypass template",   type: "slop",  e1: 36, e2: 25, e3: 68, observedComposite: 44 },
  { id: 65, name: "curl HSTS bypass",       type: "legit", e1: 30, e2: 44, e3: 50, observedComposite: 48 },
  { id: 66, name: "Firefox UAF",            type: "legit", e1: 30, e2: 64, e3: 55, observedComposite: 61 },
  { id: 69, name: "Linux NULL deref",       type: "legit", e1: 30, e2: 74, e3: 50, observedComposite: 64 },
  { id: 71, name: "IDOR with HTTP pairs",   type: "legit", e1: 34, e2: 87, e3: 46, observedComposite: 70 },
  { id: 73, name: "OpenSSL cert bypass",    type: "legit", e1: 30, e2: 82, e3: 68, observedComposite: 76 },
];
...
describe("Sprint 12 A1: E3 substance gate", () => {
  ...
  it("calibration summary (always passes, prints before/after table)", () => {
    ...
    lines.push(
      pad("ID", 4) + pad("Type", 7) + pad("Name", 24) +
      padR("E1", 4) + padR("E2", 4) + padR("E3", 4) +
      padR("OBS", 6) + padR("OFF", 6) + padR("ON", 6) + padR("Δ", 6) + "  Gate?",
    );
    lines.push("-".repeat(92));
    const off: number[] = [];
    const on: number[] = [];
    for (const b of BENCH) {
      ...
      const fired = onR.overridesApplied.some((o) => o.startsWith("E3_SUBSTANCE_GATE"));
      off.push(offC);
      on.push(onC);
      const d = onC - offC;
      lines.push(
        pad(b.id, 4) + pad(b.type, 7) + pad(b.name, 24) +
        padR(b.e1, 4) + padR(b.e2, 4) + padR(b.e3, 4) +
        padR(b.observedComposite, 6) + padR(offC, 6) + padR(onC, 6) +
        padR(d >= 0 ? `+${d}` : `${d}`, 6) + "  " + (fired ? "yes" : "no"),
      );
    }
    lines.push("-".repeat(92));
    const slopIdx = BENCH.map((b, i) => (b.type === "slop" ? i : -1)).filter((i) => i >= 0);
    const legitIdx = BENCH.map((b, i) => (b.type === "legit" ? i : -1)).filter((i) => i >= 0);
    ...
    lines.push(`gate OFF: highest slop=${offSlopMax}  lowest legit=${offLegitMin}  gap=${offLegitMin - offSlopMax}`);
    lines.push(`gate ON : highest slop=${onSlopMax}  lowest legit=${onLegitMin}  gap=${onLegitMin - onSlopMax}`);
    lines.push(`Δ gap   : ${(onLegitMin - onSlopMax) - (offLegitMin - offSlopMax)} points`);
    lines.push(`Sprint 12 Part A end-state target: gap ≥ 15, highest slop < 35`);
    lines.push(`A1 alone (this commit): closes ~10 of the ~11 needed points; A2/A3 to follow.`);
  ...
  it("with the gate ENABLED, slop reports drop and legit reports stay put", () => {
    ...
    for (const b of BENCH) {
      ...
      if (b.type === "slop") {
        ...
        expect(newComposite, `${b.name}: should not rise`).toBeLessThanOrEqual(b.observedComposite + 1);
      } else {
        // Legit reports should not drop by more than a couple of points.
        expect(newComposite, `${b.name}: should not collapse`).toBeGreaterThanOrEqual(b.observedComposite - 3);
      }
    }
  });
  ...
  it("closes the slop/legit gap from ~4 points to ≥10 points", () => {
    process.env.VULNRAP_E3_SUBSTANCE_CAP = "true";
    const slopMax = Math.max(...BENCH.filter((b) => b.type === "slop").map(score));
    const legitMin = Math.min(...BENCH.filter((b) => b.type === "legit").map(score));
    const gap = legitMin - slopMax;
    // eslint-disable-next-line no-console
    console.log(`[E3 gate] slop max=${slopMax} legit min=${legitMin} gap=${gap}`);
  ...
  it("includes an E3_SUBSTANCE_GATE override note when the cap fires", () => {
    ...
    expect(result.overridesApplied.some((o) => o.startsWith("E3_SUBSTANCE_GATE"))).toBe(true);
  });
  ...
  it("does NOT fire when E2 is healthy", () => {
    ...
    expect(result.overridesApplied.some((o) => o.startsWith("E3_SUBSTANCE_GATE"))).toBe(false);
  });
});
