...
describe.skipIf(!HAVE_CURATED)(
  "Sprint 9 v3 — Gate 2: curated dataset distribution",
  () => {
...
describe.skipIf(!HAVE_CURATED)(
  "Sprint 9 v3 — Gate 3: known curl-slop H1 IDs",
  () => {
    it("all 25 known curl-slop reports score below 50 composite", async () => {
      ...
      if (found.length === 0) {
        ...
        console.warn(
          "[gate3] curated dataset present but contains none of the 25 known curl-slop IDs",
        );
        return;
      }
      const failures = found.filter((f) => f.score >= 50);
      expect({ failures, total: found.length }).toEqual({
        failures: [],
        total: found.length,
      });
    });
