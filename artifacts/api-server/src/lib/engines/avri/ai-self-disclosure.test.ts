...
describe("addAiSelfDisclosurePhrase + JSON loader", () => {
  ...
  it("appends a new pattern that the detector starts firing on the next call", () => {
    const text = "This report was generated by Mistral-7B via internal tooling.";
    ...
    const result = addAiSelfDisclosurePhrase(
      ...
      undefined,
      { reviewer: "reviewer@example.com", rationale: "Spotted in 4 recent slop reports." },
    );
  ...
  it("is idempotent on id: re-adding the same id returns added=false and leaves the file untouched", () => {
    const first = addAiSelfDisclosurePhrase("dup_pattern", "\\bfoobar\\b", undefined, {});
    expect(first.added).toBe(true);
    const before = getAiSelfDisclosurePhrases();
    const second = addAiSelfDisclosurePhrase("dup_pattern", "\\bsomething-else\\b", undefined, {});
  ...
  it("rejects malformed input with field-prefixed errors", () => {
    expect(() => addAiSelfDisclosurePhrase("", "\\bfoo\\b", undefined, {})).toThrow(/^id /);
    expect(() => addAiSelfDisclosurePhrase("bad id with spaces", "\\bfoo\\b", undefined, {})).toThrow(/^id /);
    expect(() => addAiSelfDisclosurePhrase("ok_id", "", undefined, {})).toThrow(/^pattern /);
    expect(() => addAiSelfDisclosurePhrase("ok_id", "x".repeat(501), undefined, {})).toThrow(/^pattern /);
    expect(() => addAiSelfDisclosurePhrase("ok_id", "(unbalanced", undefined, {})).toThrow(/^pattern /);
    expect(() => addAiSelfDisclosurePhrase("ok_id", "\\bfoo\\b", "Z", {})).toThrow(/^flags /);
    expect(() =>
      addAiSelfDisclosurePhrase("ok_id", "\\bfoo\\b", undefined, { rationale: "ab" }),
    ).toThrow(/^rationale /);
  });
  ...
  it("preserves the bounded-penalty contract after adding extra phrases (Task #429)", () => {
    ...
    addAiSelfDisclosurePhrase("extra_one", "\\bfirst-extra-marker\\b", undefined, {});
    addAiSelfDisclosurePhrase("extra_two", "\\bsecond-extra-marker\\b", undefined, {});
    addAiSelfDisclosurePhrase("extra_three", "\\bthird-extra-marker\\b", undefined, {});
    const text = `This report was prepared using an AI security assistant.
...
first-extra-marker second-extra-marker third-extra-marker.`;
  ...
  it("ignores entries with malformed regex on disk so one bad row can't disable the detector", async () => {
    ...
    expect(detectAiSelfDisclosure("hit another-marker here").detected).toBe(true);
  });
});
