...
describe("createRemovalImpactCache", () => {
  ...
  it("treats different productionScanLimit values as distinct keys", () => {
    ...
    expect(cache.get("foo", 2000, "v1")?.response).toEqual({ payload: "limit-2k" });
    expect(cache.get("foo", 5000, "v1")?.response).toEqual({ payload: "limit-5k" });
  });
  ...
  it("treats different phrase strings as distinct keys", () => {
    ...
    expect(cache.get("foo", 2000, "v1")?.response).toEqual({ payload: "for-foo" });
    expect(cache.get("bar", 2000, "v1")?.response).toEqual({ payload: "for-bar" });
  });
  ...
  it("set() refreshes an existing entry in place", () => {
    ...
    expect(cache.size()).toBe(1);
    expect(cache.get("foo", 2000, "v1")?.response).toEqual({ payload: "second" });
  ...
  it("evicts least-recently-used entries when the 64-entry cap is exceeded", () => {
    ...
    for (let i = 6; i < 70; i++) {
      expect(cache.get(`phrase-${i}`, 2000, "v1")?.response).toEqual({ payload: `p${i}` });
    }
  });
  ...
  it("a get() touches the entry so it survives subsequent eviction pressure", () => {
    ...
    expect(cache.get("p-new", 2000, "v1")?.response).toEqual({ payload: "new" });
  });
});
