// Task #378 — reviewer-tunable compaction window for the curated
// dataset-history.json trend.
//
// Companion to archetype-history-config.ts. Task #264 added an
// older-than-window rollup to the persisted dataset trend so the file
// stays small over many months of runs, but the window was controlled
// only by DATASET_HISTORY_COMPACT_DAYS, which means tuning the
// trend-resolution / storage tradeoff required a redeploy. This module
// introduces a small persisted JSON config that the calibration
// dashboard reads/writes via /api/test/dataset-history/config so
// reviewers can adjust the window inline.
//
// Resolution order for the effective window mirrors archetype-history:
//   1. DATASET_HISTORY_COMPACT_DAYS env var (when set to a positive
...
const DEFAULT_CONFIG_PATH = path.resolve(
  process.cwd(),
  "artifacts/api-server/data/dataset-history-config.json",
);
...
function configPath(): string {
  return process.env.DATASET_HISTORY_CONFIG_PATH ?? DEFAULT_CONFIG_PATH;
}

// Cached so getEffectiveCompactAfterDays() (called on every
// appendDatasetCohortSnapshots) stays cheap. Keyed by path so a test
// that overrides DATASET_HISTORY_CONFIG_PATH cannot read another test's
// value.
...
function loadPersistedSync(): number | null {
  ...
  try {
    ...
    if (code !== "ENOENT") {
      console.warn(
        `[dataset-history-config] failed to read ${p} (${code ?? (err as Error)?.message ?? "unknown"}); ignoring persisted compaction window`,
      );
    }
