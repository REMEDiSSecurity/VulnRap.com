...
CVSS: 9.8.`,
  ...
  // Task #303 — one fabricated fixture per new bounds-based structural
  // detector. Each fixture trips its target marker (asserted in
  // hallucination-detector.test.ts) and pairs it with two other structural
  // markers plus a magic PID and an LLM-tell prose preamble so the overall
  // composite lands inside the calibrated T4 band (0..35) under BOTH the
  // AVRI-on and AVRI-off scoring paths. Calibration of the cohort scores
  // is asserted in hallucination-detector.test.ts (Task #303 block).
  {
    ...
    // to #4) so the structural-fabrication marker count reaches 3 and the
    // hallucination override hits its highest tier under the legacy engines
    // path. Magic PID 12345 + the structural primary trips fabricated_pid.
    text: `# Heap-use-after-free in libserver request handler (CVE-2099-30311)
...
Working PoC in exploit.py (not attached). Severity: Critical. CVSS: 10.0.`,
    expectedComposite: [0, 35],
    // Engine 2 substance score lands in the 19..37 range across AVRI on/off
    // for these three fixtures — the AVRI-off legacy substance scorer gives
    // a small amount of credit for the structural ASan-shaped block (file-
    // less frames, ERROR header, READ-of-size). [0, 40] absorbs that
    // without conflating with T1 substance scores (T1 floor ≥ 60).
  ...
  },
  {
    ...
    // structural-fabrication marker count reaches 3. A second `==12345==`
    // header in the freed-by block is a magic PID; combined with the
    // structural primary it trips fabricated_pid.
    text: `# Heap-use-after-free in libfoo parser (CVE-2099-30312)

Generated by automated AI-assisted analysis; the model's confidence on
this finding was rated "very high" so I am submitting as Critical. I have
not produced a runnable C reproducer; exploit.py is private and embargoed
under responsible disclosure.

\`\`\`asan
==99999999==ERROR: AddressSanitizer: heap-use-after-free on address 0xa1c0
READ of size 8 at 0xa1c0 thread T99999
    #0 0x4abf in foo_finalize+0x100
    #1 0x4abf in foo_main+0x200
    #3 0x4abf in main+0x300
==12345== freed by thread T99999 here:
...
Severity: Critical. CVSS: 10.0.`,
  ...
  },
  {
    ...
    // `frame_numbering_gaps` (jump from #1 to #3) so the structural-
    // fabrication marker count reaches 3. Magic PID 12345 + structural
    // primary trips fabricated_pid.
    text: `# Heap-use-after-free in libbar (CVE-2099-30313)

Generated by automated AI-assisted code analysis; the analyzer rated
confidence "very high" so I am submitting as Critical. I have not
produced a runnable C reproducer — exploit.py is private and embargoed.

\`\`\`asan
==12345==ERROR: AddressSanitizer: heap-use-after-free on address 0xa1c0
READ of size 8 at 0xa1c0 thread T0
    #0 0x4001 in process_chunk+0x100
    #1 0x4002 in dispatch+0x200
    #3 0x4003 in main+0x300
freed by thread T0 here:
    #0 0x4004 in __interceptor_free
allocated by thread T0; region size: 2
\`\`\`
...
Working PoC in exploit.py (not attached). Severity: Critical. CVSS: 10.0.`,
  ...
  // Task #434 — fabricated fixture exercising the new
  // `malformed_shadow_bytes` structural detector. The pasted ASan excerpt
  // contains a "Shadow bytes around the buggy address:" header followed
  // by rows that carry only 8 hex byte pairs (real ASan emits 16), no
  // `=>` buggy-address marker, and no legend at the bottom — three
  // independent invariant violations, any one of which is enough to fire
  // the detector. The structural-marker count reaches 3 (the new shadow-
  // bytes marker plus `round_function_offsets` from the +0x100/+0x200/
  // +0x300 offsets and `frame_numbering_gaps` from the #1→#3 jump);
  // combined with the magic PID 12345 and the AI-assisted preamble the
  // composite lands inside the calibrated T4 [0, 35] band under both
  // AVRI on and AVRI off.
  {
    id: "T4-17-fake-shadow-bytes",
    tier: "T4_HALLUCINATED",
    condemnedBy: "HallucinationDetector",
    text: `# Heap-use-after-free in libqux event dispatcher (CVE-2099-30315)
...
==12345==ERROR: AddressSanitizer: heap-use-after-free on address 0xa1c0
READ of size 8 at 0xa1c0 thread T0
    #0 0x4001 in dispatch_event+0x100
    #1 0x4002 in event_loop+0x200
    #3 0x4003 in main+0x300
Shadow bytes around the buggy address:
  0x0c047fff7f80: 00 00 00 00 00 00 00 00
  0x0c047fff7f90: fa fa fa fa fa fa fa fa
  0x0c047fff7fa0: fd fd fd fd fd fd fd fd
  0x0c047fff7fb0: fd fd fd fd fd fd fd fd
\`\`\`
...
Working PoC in exploit.py (not attached). Severity: Critical. CVSS: 10.0.`,
  ...
  // Task #608 — fabricated fixture exercising the new
  // `shadow_byte_class_mismatch` semantic detector. The pasted ASan
  // excerpt declares `heap-use-after-free` in the header but paints the
  // bracketed buggy byte on the `=>` row as `[fa]` (heap left redzone) —
  // a value that contradicts the declared bug class (real ASan tags a
  // UAF buggy byte with `fd`, "Freed heap region"). The grid shape is
  // otherwise intact (16 byte pairs per row, `=>` marker, legend present)
  // so the Task #434 `malformed_shadow_bytes` shape detector stays quiet
  // and only the new semantic check fires. The structural-marker count
  // reaches 2 (the new mismatch marker plus `round_function_offsets`
  // from the +0x100/+0x200/+0x300 offsets), so `structural_fabrication`
  // crosses its 2-marker threshold; combined with the magic PID 12345
  // and the AI-assisted preamble the composite lands inside the
  // calibrated T4 [0, 35] band under both AVRI on and AVRI off.
  {
    id: "T4-18-fake-shadow-byte-mismatch",
    tier: "T4_HALLUCINATED",
    condemnedBy: "HallucinationDetector",
    text: `# Heap-use-after-free in libqux session manager (CVE-2099-30316)
...
==12345==ERROR: AddressSanitizer: heap-use-after-free on address 0xa1c0
READ of size 8 at 0xa1c0 thread T0
    #0 0x4001 in session_lookup+0x100
    #1 0x4002 in handle_request+0x200
    #2 0x4003 in worker_loop+0x300
Shadow bytes around the buggy address:
