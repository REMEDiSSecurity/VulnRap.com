...
const submitLimiter = rateLimit({
  ...
  legacyHeaders: false,
  message: {
    error: "Too many phrase suggestions from this IP. Try again later.",
  },
});
  ...
  async (req, res): Promise<void> => {
    try {
      ...
      if (text.length < MIN_TEXT_LENGTH) {
        res.status(400).json({
          error: `Phrase must be at least ${MIN_TEXT_LENGTH} characters.`,
        });
        return;
      }
      if (text.length > MAX_TEXT_LENGTH) {
        res.status(400).json({
          error: `Phrase must be at most ${MAX_TEXT_LENGTH} characters.`,
        });
        return;
      }
      if (!ALLOWED_CATEGORIES.has(rawCategory)) {
        res.status(400).json({
          error: "category must be 'handwavy' or 'ai-self-disclosure'.",
        });
        return;
      }
      ...
      if (dup.length > 0) {
        res.status(200).json({
          ...
          duplicate: true,
          message:
            "You already suggested this phrase recently — thanks, we'll review it.",
        });
        return;
      }
      ...
      res.status(500).json({
        error: "Failed to submit phrase suggestion. Please try again later.",
      });
    }
  },
);
