...
4. Stay non-technical. Any acronym (CWE, PoC, AVRI) gets spelled out the first time it appears on screen *and* in the voiceover.
...
- **B-roll:** Caption card slides in: *"Slop = low-effort or AI-fabricated reports."* Stays on screen for 3 seconds.
- **Word count:** 26 words.
...
- **B-roll:** Three small annotation arrows appear, one per claim: "no such CVE", "no such file", "steps don't run." Each arrow appears synced to the matching word in the VO. The first arrow's tooltip also spells out *"CVE = Common Vulnerabilities and Exposures."*
- **Word count:** 36 words.
...
- **B-roll:** As "AVRI" lights up, a small caption reads *"AVRI = Adversarial Vulnerability Reasoning Index."*
- **Word count:** 30 words.
...
- **B-roll:** Caption card on first appearance of CWE: *"CWE = Common Weakness Enumeration, the standard list of bug categories."*
- **Word count:** 28 words.
...
- **B-roll:** End card: VulnRap wordmark, URL, and the line *"Free. Open methodology. No login."*
- **Word count:** 22 words.
...
2. **Upload locations**
   ...
   - Mirror: an unlisted YouTube upload at the same time, for users who prefer YouTube's player or want captions/translations later. Link from the `/transparency` page footer, *not* from the home page.
...
5. **Alt text and accessibility**
   - `aria-label` on the `<video>` element: *"90-second walkthrough of how VulnRap scores a vulnerability report. Captions available."*
