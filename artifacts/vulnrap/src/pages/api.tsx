import { Code, ExternalLink, FileText, Search, Shield, Activity, MessageSquare, Heart, Terminal, Copy, Check, ChevronDown, ChevronUp, Plug, Bot, Download, Image as ImageIcon, Server, Bookmark } from "lucide-react";
...
func main() {
    ...
    log.Printf("slop=%d/%s confidence=%.2f", res.SlopScore, res.SlopTier, res.Confidence)
      ...
      <div className="space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          Drag the button below onto your bookmarks bar. Click it on any page (a HackerOne report, a Bugcrowd submission, an email, …) to open <code className="font-mono text-xs text-foreground">/check</code> with the currently selected text pre-filled. No extension to install, no permissions to grant.
        </p>

        <Card className="glass-card rounded-xl" data-testid="card-bookmarklet">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm flex items-center gap-2">
              <Badge variant="outline" className="border-purple-500 text-purple-500 text-[10px] font-mono uppercase">Bookmarklet</Badge>
              <code className="text-primary font-mono text-xs">javascript:(...)</code>
            </CardTitle>
            <CardDescription className="mt-1">
              One-line IIFE that grabs <code className="font-mono text-xs text-foreground">window.getSelection()</code> and opens{" "}
              <code className="font-mono text-xs text-foreground">vulnrap.com/check?text=&lt;encoded&gt;</code> in a new tab. Selections longer than 50,000 chars are truncated to stay under URL length limits. Does not auto-submit — you always click <strong>Check</strong> yourself.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            <div className="rounded-lg bg-muted/30 px-4 py-3 text-xs text-muted-foreground leading-relaxed space-y-1.5">
              ...
              <p>If your browser blocks dragging, create a new bookmark and paste the contents of <code className="font-mono">sdks/bookmarklet/vulnrap.bookmarklet.url</code> as the URL.</p>
            </div>
            <a
            ...
            <p className="text-[11px] text-muted-foreground/60 italic">
              Repo path: <code className="font-mono">sdks/bookmarklet/</code> — regenerate with <code className="font-mono">pnpm --filter @workspace/scripts run generate:bookmarklet</code>.
            </p>
          </CardContent>
        </Card>
      </div>
