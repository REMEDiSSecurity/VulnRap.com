  ...
  return (
    <div className="space-y-1.5">
        ...
        <div className="text-right shrink-0 tabular-nums">
          {clamped ? (
            <>
              ...
              <span className="font-bold" style={{ color: hex.fill }}>
                {effective}
              </span>
            </>
          ) : (
      ...
      <div
        ...
        style={{
          background: hex.track,
          boxShadow: `inset 0 0 0 1px ${hex.ring}`,
        }}
      >
        {clamped && (
          <div
      ...
      <div className="flex items-baseline justify-between gap-2 text-[10.5px] sm:text-[11px] font-mono text-slate-500 leading-tight">
        ...
        <span className="shrink-0 tabular-nums">
          {inverted ? (
            <>
              0.{weight.toString().padStart(2, "0")} {`\u00D7`} (100 {`\u2212`}{" "}
              {raw}) ={" "}
            </>
          ) : (
            <>
              0.{weight.toString().padStart(2, "0")} {`\u00D7`}{" "}
              {clamped ? effective : raw} ={" "}
            </>
          )}
...
export default function NotFound() {
  ...
  const displayPath =
    sanitized.length < rawPath.length ? sanitized + "\u2026" : sanitized;
    ...
    <div className="min-h-[70vh] py-10 sm:py-16 px-4 flex items-start justify-center">
      <div className="w-full max-w-2xl space-y-6">
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(135deg, rgba(15,23,42,0.75) 0%, rgba(15,23,42,0.45) 100%)",
            boxShadow:
          ...
          <div className="relative space-y-5">
            ...
            <div className="space-y-2">
              ...
              <div className="text-[12px] sm:text-[13px] text-slate-400 italic leading-snug">
                No proof of concept. No payload. No file path. No CVSS. No CWE
                cited.
                <br />
                Just{" "}
            ...
            <div className="space-y-3.5 pt-1">
              ...
              <div className="space-y-1">
                ...
                >
                  ...
                  substance gate {`\u00B7`} E2 {`<`} 30 {`\u2192`} would cap E3
                  at 42
                </div>
              </div>
            </div>
            ...
            <div className="pt-2 mt-2 border-t border-slate-800 flex items-end justify-between gap-4 flex-wrap">
              <div>
                ...
                <div className="text-[11px] font-mono text-slate-500 mt-1">
                  {e1Contrib.toFixed(2)} + {e2Contrib.toFixed(2)} +{" "}
                  {e3Contrib.toFixed(2)} ={" "}
                  {(e1Contrib + e2Contrib + e3Contrib).toFixed(2)} {`\u2248`}{" "}
                  {composite}
                </div>
              </div>
        ...
        <div className="space-y-3 px-1">
          <p className="text-slate-300 text-[14px] sm:text-[15px] leading-relaxed">
            <span className="font-bold text-cyan-400">VulnRap</span> scores
            vulnerability reports the same way we just scored that URL{": "}
            <span className="font-mono text-slate-200">5%</span> AI Authorship,{" "}
            <span className="font-mono text-slate-200">60%</span> Technical
            Substance, <span className="font-mono text-slate-200">35%</span> CWE
            Coherence{". "}
            About <span className="font-bold text-slate-200">78%</span> of
            submissions PSIRT teams see right now look{" "}
            <span className="italic text-slate-400">
              a lot like the report above
            </span>
            {": "}
            confidently written, technically empty.
          </p>
          <p className="text-slate-400 text-[13px] sm:text-[14px] leading-relaxed">
            We help triagers spend their time on the other 22%. Paste a report,
            get a composite score and a triage action in seconds {`\u2014`} no
            signup, no upload, nothing stored unless you ask.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2.5 pt-1">
          <Button
            asChild
            className="bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-semibold flex-1"
          >
            <Link to="/">
