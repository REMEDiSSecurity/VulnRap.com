...
import {
  ...
  useGetSlopDistribution,
  useGetVisitorStats,
  getGetStatsQueryKey,
  getGetRecentActivityQueryKey,
  getGetSlopDistributionQueryKey,
...
function HeadlineNarrative({
  ...
  const cleanCount =
    distribution?.buckets
      ...
      .reduce((sum, b) => sum + b.count, 0) ?? 0;
  const slopPct =
    totalReports > 0 ? Math.round((slopCount / totalReports) * 100) : 0;
  const cleanPct =
    totalReports > 0 ? Math.round((cleanCount / totalReports) * 100) : 0;
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="text-sm text-muted-foreground leading-relaxed">
          ...
          {slopPct > 0 && (
            <>
              {" "}
              <span className="text-red-400 font-medium">{slopPct}%</span>{" "}
              flagged as likely slop or worse.
            </>
          )}
          {cleanPct > 0 && (
            <>
          ...
          {dupRate > 0 && (
            <>
              ...
              <span className="text-foreground font-mono">{dupRate}%</span>.
            </>
          )}
        </div>
      </div>
    ...
    <div className="max-w-6xl mx-auto space-y-8">
      <div className="pb-6">
        ...
        <p className="text-muted-foreground mt-2">
          Aggregate metrics across the VulnRap validation network.
        </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          secondDetail={formatNumber(
            stats?.reportsByMode?.similarity_only || 0,
          )}
        ...
        <StatCard
          ...
          secondDetail={formatNumber(
            (stats?.totalReports || 0) - (stats?.duplicatesDetected || 0),
          )}
        ...
        <StatCard
          ...
          secondDetail={
            stats?.avgSlopScore
              ? `${stats.avgSlopScore.toFixed(1)} / 100`
              : "0 / 100"
          }
        ...
        <StatCard
          ...
          secondDetail={
            stats && stats.reportsThisWeek && stats.reportsThisWeek > 0
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Reports by tier &mdash; color-coded by classification
                </CardDescription>
              </div>
                  ...
                  {distribution.buckets.map((bucket, i) => {
                    ...
                    const pct =
                      distribution.totalReports > 0
                        ? Math.round(
                            (bucket.count / distribution.totalReports) * 100,
                          )
                        : 0;
                    ...
                    return (
                      <button
                        key={i}
                        type="button"
                        className="flex-1 flex flex-col items-center group relative h-full cursor-default focus:outline-none"
                        aria-label={`${bucket.count} reports, ${pct}% of total, ${bucket.label}`}
                      >
                        <div className="w-full flex-1 relative">
                          ...
                          {bucket.count > 0 && (
                            <div
                              className={cn(
                                "absolute left-1/2 -translate-x-1/2 font-mono text-[10px] font-bold transition-opacity",
                                tierColor.text,
                              )}
                              style={{
                                bottom: `${Math.max(heightPct, 3) + 2}%`,
                              }}
                            >
                              {bucket.count}
                            </div>
                        ...
                        <div className="absolute bottom-full mb-2 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity glass-card text-popover-foreground text-xs px-3 py-2 rounded-lg pointer-events-none z-10 glow-border space-y-0.5">
                          ...
                          <div className="text-muted-foreground/60 text-[10px]">
                            {bucket.label} ({bucket.min}&ndash;{bucket.max})
                          </div>
                        </div>
                      </button>
                    );
                  ...
                  {distribution.buckets.map((bucket, i) => {
                    ...
                    return (
                      <div key={i} className="flex-1">
                        <div
                          className={cn("h-1.5 rounded-full", tierColor.bar)}
                          style={{
                ...
                {activity.recentReports.map((report) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={cn(
                          "p-3 glass-card rounded-lg hover:border-primary/20 transition-all group border-l-2",
                          tierColor.border,
                        )}
                      >
                        <div className="flex justify-between items-start mb-2">
                          <div className="font-mono text-sm text-primary glow-text-sm group-hover:glow-text transition-all">
                            #{report.id}
                          </div>
