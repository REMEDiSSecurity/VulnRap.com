...
export function DevelopersAndAgentsSection() {
  return (
    <div
      className="glass-card rounded-xl p-4 sm:p-6 space-y-5"
      data-scroll-fade
    >
      <div className="space-y-1">
        <span className="eyebrow-label">
          Section 05 · Developers &amp; Agents
        </span>
        ...
        <p className="text-xs text-muted-foreground leading-relaxed max-w-3xl pt-1">
          VulnRap is a free, no-account REST API. Pipe inbound vulnerability
          reports through it for automatic slop / quality / validity scoring,
          then feed your triage outcome back so the platform keeps getting
          better. Built for security tools, triage bots, and the new wave of AI
          coding agents.
        </p>
      </div>
          ...
          <ol className="space-y-2 text-xs text-muted-foreground leading-relaxed">
            <li className="flex gap-2">
              ...
              <span>
                ...
                — submit raw text, a file, or a URL (allowlisted hosts). Get
                back a slop score, quality score, validity score, section-hash
                matches, and an integer report id.
              </span>
            </li>
            <li className="flex gap-2">
  ...
  -F "showInFeed=true"`}
        ...
        <div className="rounded-xl bg-violet-500/5 border border-violet-500/20 p-4 space-y-3">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            We publish an{" "}
            <span className="font-mono text-foreground">agents.md</span> at the
            site root — the same convention coding agents already look for. It's
            the integration manual: recommended defaults, a 30-second loop, what
            to surface to your human, and (importantly) how to send feedback so
            VulnRap learns from every triage decision your agent makes.
          </p>
          <ul className="space-y-1.5 text-xs text-muted-foreground leading-relaxed">
            <li className="flex gap-2">
              <span className="text-violet-400 mt-0.5">&#8226;</span>Default to{" "}
              <span className="font-mono text-foreground">showInFeed=true</span>{" "}
              +{" "}
              <span className="font-mono text-foreground">
                contentMode=full
              </span>{" "}
            ...
            <li className="flex gap-2">
              <span className="text-violet-400 mt-0.5">&#8226;</span>Always call{" "}
              <span className="font-mono text-foreground">
                POST /api/feedback
              </span>{" "}
              after the human acts — that ground truth is how scoring weights
              improve.
            </li>
            <li className="flex gap-2">
              <span className="text-violet-400 mt-0.5">&#8226;</span>No account,
