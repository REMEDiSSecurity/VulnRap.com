...
import { UploadCloud, Shield, FileText, Loader2, CheckCircle, XCircle, Search, Zap, Eye, HelpCircle, Lock, Fingerprint, ShieldCheck, Volume2, VolumeX, ClipboardPaste, Clock, ExternalLink, Info, X, Link2, ChevronDown, Play, AlertTriangle, Trash2, Mail, BrainCircuit, ShieldOff, Users, TrendingUp, TrendingDown, Code, Bot, Terminal } from "lucide-react";
...
function DevelopersAndAgentsSection() {
  return (
    <div className="glass-card rounded-xl p-4 sm:p-6 space-y-5" data-scroll-fade>
      <div className="space-y-1">
        <span className="eyebrow-label">Section 05 · Developers &amp; Agents</span>
        <h2 className="text-lg font-bold flex items-center gap-2">
          <Code className="w-5 h-5 text-primary" />
          Public API &amp; AI Agent Manual
        </h2>
        <p className="text-xs text-muted-foreground leading-relaxed max-w-3xl pt-1">
          VulnRap is a free, no-account REST API. Pipe inbound vulnerability reports through it for
          automatic slop / quality / validity scoring, then feed your triage outcome back so the platform
          keeps getting better. Built for security tools, triage bots, and the new wave of AI coding agents.
        </p>
      </div>
        ...
        <div className="rounded-xl bg-primary/5 border border-primary/20 p-4 space-y-3">
          ...
          <ol className="space-y-2 text-xs text-muted-foreground leading-relaxed">
            <li className="flex gap-2">
              ...
              <span><span className="font-mono text-foreground">POST /api/reports</span> — submit raw text, a file, or a URL (allowlisted hosts). Get back a slop score, quality score, validity score, section-hash matches, and an integer report id.</span>
            </li>
            <li className="flex gap-2">
              <span className="text-primary font-mono mt-0.5">2.</span>
              <span><span className="font-mono text-foreground">GET /api/reports/&#123;id&#125;/triage-report</span> — Markdown summary you can paste straight into a Jira / ServiceNow / chat reply.</span>
            </li>
            <li className="flex gap-2">
              <span className="text-primary font-mono mt-0.5">3.</span>
              <span><span className="font-mono text-foreground">POST /api/feedback</span> — one short note about what actually happened (real / dup / fabricated). This is how the engines recalibrate.</span>
            </li>
          </ol>
          <div className="pt-1">
...
{`curl -X POST https://vulnrap.com/api/reports \\
  ...
  -F "showInFeed=true"`}
        ...
        <div className="rounded-xl bg-violet-500/5 border border-violet-500/20 p-4 space-y-3">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            We publish an <span className="font-mono text-foreground">agents.md</span> at the site root —
            the same convention coding agents already look for. It's the integration manual: recommended
            defaults, a 30-second loop, what to surface to your human, and (importantly) how to send
            feedback so VulnRap learns from every triage decision your agent makes.
          </p>
          <ul className="space-y-1.5 text-xs text-muted-foreground leading-relaxed">
            <li className="flex gap-2"><span className="text-violet-400 mt-0.5">&#8226;</span>Default to <span className="font-mono text-foreground">showInFeed=true</span> + <span className="font-mono text-foreground">contentMode=full</span> so the analysis is enriched and benefits the community.</li>
            <li className="flex gap-2"><span className="text-violet-400 mt-0.5">&#8226;</span>Always call <span className="font-mono text-foreground">POST /api/feedback</span> after the human acts — that ground truth is how scoring weights improve.</li>
            <li className="flex gap-2"><span className="text-violet-400 mt-0.5">&#8226;</span>No account, no API key. PII scrubbing happens before storage.</li>
          </ul>
          <div className="flex flex-wrap gap-2 pt-1">
            <a
              href="/agents.md"
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-muted-foreground/70 pt-1">
        <span className="inline-flex items-center gap-1.5"><Shield className="w-3 h-3 text-primary/60" />No auth required</span>
        <span className="inline-flex items-center gap-1.5"><Zap className="w-3 h-3 text-amber-400/70" />30 analyses / 15 min / IP</span>
        <span className="inline-flex items-center gap-1.5"><Lock className="w-3 h-3 text-cyan-400/70" />PII scrubbed before storage</span>
        <span className="inline-flex items-center gap-1.5"><FileText className="w-3 h-3 text-violet-400/70" />Markdown triage export</span>
      </div>
    </div>
  );
}
