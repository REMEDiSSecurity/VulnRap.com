...
import { BarChart3, Database, Layers, Tag, TrendingUp, Activity } from "lucide-react";
...
function tierClass(tier: string): string {
  return TIER_COLORS[tier] || "text-muted-foreground border-border/40 bg-muted/10";
}
...
function HBar({
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="w-40 shrink-0 truncate text-xs font-mono text-foreground/90" title={label}>
        {label}
      </div>
...
function VolumeChart({ series }: { series: Array<{ date: string; count: number }> }) {
  if (series.length === 0) {
    return <div className="h-40 flex items-center justify-center text-sm text-muted-foreground">No volume data yet.</div>;
  }
  ...
  return (
    <div className="space-y-3">
          ...
          return (
            ...
            >
              ...
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 opacity-0 group-hover:opacity-100 transition-opacity glass-card text-popover-foreground text-[10px] px-2 py-1 rounded pointer-events-none z-10 whitespace-nowrap">
                ...
                <div className="font-mono text-primary">{point.count} report{point.count === 1 ? "" : "s"}</div>
              </div>
            </div>
          );
        })}
      ...
      <div className="flex items-center justify-between text-[10px] font-mono text-muted-foreground/60 border-t border-border/30 pt-2">
        ...
        <span>{total.toLocaleString()} reports across {series.length} day{series.length === 1 ? "" : "s"}</span>
    ...
    <div className="max-w-6xl mx-auto space-y-8">
      <div className="pb-6">
        ...
        <p className="text-muted-foreground mt-2">
          Already-aggregated metrics across every report VulnRap has scored. No individual report content is exposed here — only counts, totals, and time series.
        </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="glass-card rounded-xl">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs uppercase tracking-wider text-muted-foreground font-bold">Total Reports</span>
        ...
        <Card className="glass-card rounded-xl">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs uppercase tracking-wider text-muted-foreground font-bold">Tiers Tracked</span>
        ...
        <Card className="glass-card rounded-xl">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs uppercase tracking-wider text-muted-foreground font-bold">CWE Families</span>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Reports grouped by slop tier label.</CardDescription>
            </div>
            {totalReports > 0 && (
              <Badge variant="outline" className="text-xs font-mono">{totalReports.toLocaleString()} total</Badge>
            )}
          </div>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <div className="py-6 text-center text-sm text-muted-foreground">No reports scored yet.</div>
          ) : (
            <div className="space-y-3">
              {tiers.map((t) => {
                const pct = totalReports > 0 ? Math.round((t.count / totalReports) * 100) : 0;
                return (
                  <div key={t.tier} className="flex items-center gap-3">
                    <div className="w-32 shrink-0">
                      <Badge variant="outline" className={cn("text-[10px] font-mono", tierClass(t.tier))}>
                        {t.tier}
                      </Badge>
                    </div>
                    <div className="flex-1 h-3 rounded-sm bg-muted/30 overflow-hidden">
                      <div
                        className={cn("h-full transition-all duration-700 ease-out", tierClass(t.tier))}
                        style={{ width: `${Math.max((t.count / maxTier) * 100, 2)}%` }}
                      />
                    </div>
                    <div className="w-24 text-right text-xs font-mono text-muted-foreground tabular-nums">
                      {t.count.toLocaleString()} <span className="text-muted-foreground/50">({pct}%)</span>
                    </div>
                  </div>
                );
              })}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Card className="glass-card rounded-xl">
          <CardHeader>
            ...
            <CardDescription>Most-fired evidence signal types across the corpus.</CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              <div className="space-y-3">
              ...
              <div className="py-6 text-center text-sm text-muted-foreground">No signals fired yet.</div>
            ) : (
        ...
        <Card className="glass-card rounded-xl">
          <CardHeader>
            ...
            <CardDescription>AVRI rubric families the corpus has classified into.</CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              <div className="space-y-3">
              ...
              <div className="py-6 text-center text-sm text-muted-foreground">No CWE families classified yet.</div>
            ) : (
              <div className="space-y-2.5">
                {families.map((f) => (
                  <HBar
      ...
      <Card className="glass-card-accent rounded-xl">
        <CardHeader>
          ...
          <CardDescription>Daily report submissions over the last 90 days (oldest left, newest right).</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
