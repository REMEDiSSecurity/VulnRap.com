...
function DeltaBadge({ delta }: { delta: number | null }) {
  if (delta == null) {
    return (
      <Badge variant="outline" className="text-xs font-mono" data-testid="badge-drift-delta-empty">
  ...
  if (delta === 0) {
    return (
      <Badge variant="outline" className="text-xs font-mono" data-testid="badge-drift-delta">
        <Minus className="w-3 h-3 mr-1" />
        flat vs last week
      </Badge>
    );
  ...
  return (
    <Badge
      ...
      className={
        "text-xs font-mono " +
        (widening ? "text-emerald-400 border-emerald-500/40" : "text-amber-400 border-amber-500/40")
      }
  ...
  return (
    <div className="glass-card rounded-lg px-3 py-2 border border-border/30 text-xs space-y-1">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="font-mono font-bold text-foreground">{payload[0].value}pt</span>
      </div>
    </div>
  );
}
...
export function TransparencyDriftWidget() {
  ...
  return (
    <Card className="glass-card rounded-xl" data-testid="card-transparency-drift-widget">
      <CardHeader>
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            ...
            <CardDescription className="mt-1 max-w-2xl">
              Each week we compare the average score the engine gives the strongest reports
              (T1) against the weakest ones (T3). A wider gap means the engine is still
              telling them apart cleanly. We track this in public so anyone can see the
              platform is monitoring itself for drift.
            </CardDescription>
          </div>
      ...
      <CardContent>
        {isLoading ? (
          <Skeleton className="h-40 w-full" data-testid="skeleton-drift-widget" />
        ) : isError ? (
          <div
          ...
          <div
            ...
            data-testid="empty-drift-widget"
          >
            This week's drift aggregate hasn't been computed yet — check back once enough
            reports have come in to compare cohorts.
          </div>
        ) : (
          <div className="space-y-4" data-testid="populated-drift-widget">
            <div className="flex items-baseline gap-6 flex-wrap">
              <div>
                ...
                >
                  ...
                  <span className="text-base text-muted-foreground ml-1">pt</span>
                </div>
              </div>
            ...
            <ResponsiveContainer width="100%" height={140}>
              <AreaChart data={sparklineData} margin={{ top: 5, right: 5, bottom: 0, left: -10 }}>
                <defs>
