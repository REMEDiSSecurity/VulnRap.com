...
import { ChevronDown, ChevronUp, ShieldOff, AlertTriangle, FlaskConical, ExternalLink, Trash2 } from "lucide-react";
  ...
  return input
    ...
    .map((source) => {
      try {
        ...
        return { source, regex: null, error: e instanceof Error ? e.message : "Invalid regex" };
      }
    });
}
...
export function CustomRedactionPanel({ text, redactionDisabled }: Props) {
  ...
  const highlighted = useMemo(() => {
    ...
    for (const span of matches) {
      if (span.start > cursor) out.push({ text: text.slice(cursor, span.start), match: false });
    ...
    <Card className="glass-card rounded-xl">
      ...
      >
        <CardTitle className="flex items-center gap-2 text-sm">
          ...
          <Badge variant="outline" className="text-[10px] px-1.5 py-0 h-4 normal-case">Advanced</Badge>
          {validRules.length > 0 && (
            <Badge variant="outline" className="text-[10px] px-1.5 py-0 h-4 normal-case border-primary/40 text-primary">
        ...
        <CardDescription className="text-xs">
          Add your own regex patterns (one per line) to highlight before submission. Runs entirely in your browser.
        </CardDescription>
      </CardHeader>
      {open && (
        <CardContent className="space-y-3">
          <textarea
            ...
            placeholder={"# One regex per line. Lines starting with # are comments.\nacme-internal-[a-z0-9-]+\\.corp\nPROJECT-[A-Z]{4,}\n\\bcodename-\\w+\\b"}
            value={rulesInput}
            onChange={(e) => { setRulesInput(e.target.value); setTested(false); }}
            spellCheck={false}
            autoComplete="off"
          ...
          {invalidRules.length > 0 && (
            <div className="rounded-lg bg-destructive/10 border border-destructive/30 px-3 py-2 text-[11px] space-y-1">
              <div className="flex items-center gap-1.5 text-destructive font-medium">
                ...
                {invalidRules.length} invalid pattern{invalidRules.length === 1 ? "" : "s"}
              </div>
              <ul className="space-y-0.5 font-mono text-destructive/90">
                {invalidRules.slice(0, 3).map((r, i) => (
                  <li key={i} className="truncate"><span className="opacity-70">{r.source}</span> — {r.error}</li>
                ))}
              </ul>
            </div>
          )}
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {rulesInput.length > 0 && (
              <Button
                ...
                onClick={() => { setRulesInput(""); setTested(false); }}
          ...
          <div className="text-[11px] text-muted-foreground">
            <Link to="/redaction-examples" className="text-primary hover:underline inline-flex items-center gap-1">
          ...
          {tested && (
            <div className="space-y-2" data-testid="custom-redaction-results">
              {text.trim().length === 0 ? (
                <div className="text-[11px] text-muted-foreground">Paste report text above to test patterns against it.</div>
              ) : matches.length === 0 ? (
                ...
                <>
                  <div className="text-[11px] text-muted-foreground">
                    {matches.length} match{matches.length === 1 ? "" : "es"} highlighted below:
                  </div>
                  <pre className="max-h-64 overflow-auto rounded-lg glass-card p-3 text-[11px] font-mono whitespace-pre-wrap break-words leading-relaxed">
                    {highlighted?.map((seg, i) =>
                      seg.match ? (
                        <mark key={i} className="bg-yellow-500/30 text-yellow-100 rounded px-0.5">{seg.text}</mark>
                      ) : (
          ...
          <div className="rounded-lg bg-yellow-500/5 border border-yellow-500/20 px-3 py-2 text-[11px] text-muted-foreground leading-relaxed flex items-start gap-2">
            ...
            <span>
              Custom patterns run <strong>client-side only</strong> for previewing matches. They are never sent to the server. If you also disable
              built-in PII redaction above, your report text will be submitted unredacted — these custom rules <strong>will not</strong> sanitize it for you.
              {redactionDisabled && (
                <span className="block mt-1 text-orange-300">
                  PII redaction is currently <strong>disabled</strong>. Redact matches manually before submitting.
                </span>
              )}
            </span>
          </div>
