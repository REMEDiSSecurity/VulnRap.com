...
import { ChevronDown, ChevronUp, GitCompare, AlertTriangle } from "lucide-react";
import {
  adjustScore,
  adjustTier,
  SENSITIVITY_PRESETS,
  ...
  return (
    <div className="rounded-lg border border-border/40 bg-muted/10 p-3 flex-1 min-w-0">
      ...
      <div className="flex items-baseline gap-2 mb-2">
        <div className="text-3xl font-mono font-bold tracking-tight">{outcome.score}</div>
        <div className="text-[10px] text-muted-foreground">/ 100</div>
      </div>
      <div className={`text-sm font-bold ${tierClass} mb-3`}>{outcome.tier}</div>
      <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1">
        Top Signals After Filter
      </div>
      {outcome.topSignals.length === 0 ? (
        <div className="text-xs text-muted-foreground italic">No signals fire under this preset.</div>
      ) : (
        <ul className="space-y-1">
          {outcome.topSignals.map((s, i) => (
            <li key={i} className="text-[11px] font-mono text-muted-foreground truncate">
              · {s.type.replace(/_/g, " ")} <span className="text-primary/70">({s.weight})</span>
            </li>
          ))}
        </ul>
      )}
...
export function AbPresetComparison({
  ...
  const outcomeA = computeOutcome(presetA, canonicalScore, breakdown, humanIndicators, evidence, thresholdLow, thresholdHigh);
  const outcomeB = computeOutcome(presetB, canonicalScore, breakdown, humanIndicators, evidence, thresholdLow, thresholdHigh);
    ...
    <Card className="glass-card rounded-xl" data-testid="ab-preset-comparison">
      <CardHeader className="cursor-pointer" onClick={() => setExpanded((v) => !v)}>
        <CardTitle className="flex items-center gap-2">
          ...
          Compare Presets
          <Badge variant="outline" className="text-[10px] px-1.5 py-0 h-4">A/B</Badge>
          <span className="ml-auto">
            {expanded ? <ChevronUp className="w-4 h-4 text-muted-foreground" /> : <ChevronDown className="w-4 h-4 text-muted-foreground" />}
          </span>
        </CardTitle>
        <CardDescription>What would this report score under preset A vs preset B?</CardDescription>
      </CardHeader>
      {expanded && (
        <CardContent className="space-y-4">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1">
              <label className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1 block">Preset A</label>
              <select
                value={presetA}
                onChange={(e) => setPresetA(e.target.value as SensitivityPreset)}
              ...
              >
                {PRESETS.map((p) => (
                  <option key={p} value={p}>{SENSITIVITY_PRESETS[p].label}</option>
                ))}
              </select>
            </div>
            <div className="flex-1">
              <label className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1 block">Preset B</label>
              <select
                value={presetB}
                onChange={(e) => setPresetB(e.target.value as SensitivityPreset)}
                className="w-full rounded-md border border-border/40 bg-background/50 px-2 py-1.5 text-sm font-mono"
          ...
          {tierDiverges && (
            ...
            >
              ...
              <div className="text-xs">
                ...
                <span className="font-mono">{outcomeA.tier}</span> under {SENSITIVITY_PRESETS[presetA].label} vs{" "}
                <span className="font-mono">{outcomeB.tier}</span> under {SENSITIVITY_PRESETS[presetB].label}.
                Triage outcome is preset-sensitive for this report.
              </div>
            </div>
          )}
        </CardContent>
