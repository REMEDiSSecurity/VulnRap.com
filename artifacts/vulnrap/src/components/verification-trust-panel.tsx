...
import type { VerificationCheck, VerificationSummary } from "@workspace/api-client-react";
...
function classifySource(checkType: string): Source {
  ...
  if (checkType.startsWith("nvd_") || checkType === "verified_cve" || checkType === "invalid_cve_year") return "nvd";
...
function TrustHint({ text }: { text: string }) {
  ...
  return (
    <span className="group relative inline-flex ml-1 cursor-help">
      ...
      <span className={`pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-72 rounded-md glass-card px-3 py-2 text-xs text-popover-foreground transition-opacity z-50 glow-border text-left font-normal normal-case ${open ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}>
        {text}
      </span>
    </span>
  );
...
export function VerificationTrustPanel({ checks, summary }: { checks: VerificationCheck[]; summary?: VerificationSummary }) {
  const verifiable = checks.filter((c) => isVerifiable(c.result));
  if (verifiable.length === 0) return null;

  const verified = summary?.verified ?? verifiable.filter((c) => c.result === "verified").length;
  ...
  const ratioTone = ratio >= 0.8 ? "text-green-400" : ratio >= 0.5 ? "text-yellow-400" : "text-orange-400";
  const barTone = ratio >= 0.8 ? "bg-green-500" : ratio >= 0.5 ? "bg-yellow-500" : "bg-orange-500";

  const summaryText = notFound === 0
    ? `${verified} of ${total} referenced resources verified`
    : `${verified} of ${total} referenced resources verified, ${notFound} not found`;

  return (
    <Card className="glass-card rounded-xl" data-testid="verification-trust-panel">
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-sm">
          <ShieldCheck className="w-4 h-4 text-primary" />
          Verification
          <span className={`ml-1 font-mono text-xs ${ratioTone}`}>{verified}/{total}</span>
      ...
      <CardContent className="space-y-3 pt-0">
        <div className="space-y-1.5">
          ...
          <Progress value={pct} className="h-1.5" indicatorClassName={barTone} />
        </div>
        {sources.length > 0 && (
          <div className="flex flex-wrap gap-x-3 gap-y-1 text-[11px] font-mono text-muted-foreground" data-testid="verification-trust-source-breakdown">
            {sources.map((s) => {
              const entry = bySource.get(s)!;
              return (
                <span key={s} data-testid={`verification-trust-source-${s}`}>
                  <span className="uppercase tracking-wide">{SOURCE_LABEL[s]}</span>{" "}
                  <span className={entry.verified === entry.total ? "text-green-400" : entry.verified === 0 ? "text-orange-400" : "text-yellow-400"}>
