...
import AiSelfDisclosureAddDialog from "@/components/ai-self-disclosure-add-dialog";
...
export default function PhraseSuggestionsQueue({
  ...
  const [disclosureDialogSuggestion, setDisclosureDialogSuggestion] =
    useState<PhraseSuggestion | null>(null);
  ...
  const handleApprove = useCallback(
    async (s: PhraseSuggestion) => {
      ...
      if (s.category === "ai-self-disclosure") {
        setDisclosureDialogSuggestion(s);
        return;
      }
      setBusyId(s.id);
      try {
        ...
        toast({
          ...
          description: "Added to the curated handwavy list.",
        });
        refetch();
      } catch (err) {
        const description =
  ...
  const handleDisclosureSaved = useCallback(async () => {
    const s = disclosureDialogSuggestion;
    if (!s) return;
    setDisclosureDialogSuggestion(null);
    setBusyId(s.id);
    try {
      await updatePhraseSuggestionStatus(s.id, { status: "approved" });
      toast({
        title: "Suggestion approved",
        description: "Added to the AI self-disclosure list.",
      });
      refetch();
    } catch (err) {
      const description =
        err instanceof ApiError && err.status === 401
          ? "Reviewer token rejected — see the calibration auth banner."
          : "Regex saved but failed to mark the suggestion approved. Try again.";
      toast({
        title: "Bookkeeping failed",
        description,
        variant: "destructive",
      });
    } finally {
  ...
  }, [disclosureDialogSuggestion, toast, refetch]);
  ...
  return (
    <>
      ...
      <AiSelfDisclosureAddDialog
        open={disclosureDialogSuggestion !== null}
        suggestedText={disclosureDialogSuggestion?.text ?? ""}
        suggestionId={disclosureDialogSuggestion?.id ?? null}
        onSaved={handleDisclosureSaved}
        onCancel={() => setDisclosureDialogSuggestion(null)}
      />
    </>
  );
}
