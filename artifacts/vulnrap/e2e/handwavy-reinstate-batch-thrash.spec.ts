...
function uniquePhrases(prefix: string, count: number): string[] {
  ...
  return Array.from(
    { length: count },
    (_, i) => `task179 ${prefix} ${id} phrase ${i + 1}`,
  );
}
...
test.describe("FLAT hand-wavy phrase panel — bulk-reinstate thrash warning (Task #179, #366)", () => {
  test("batch group with a high-thrash inner phrase shows the summary banner and per-row badge", async ({
    ...
    try {
      ...
      const summary = group.getByTestId(
        "handwavy-history-batch-thrash-summary",
      );
      await expect(summary).toBeVisible();
      await expect(summary).toContainText(
        /1 of 2 selected phrases has already cycled 2\+ times/,
      );
      ...
      const thrashBadgeOnThrashy = thrashyRow.getByTestId(
        "handwavy-history-batch-thrash-badge",
      );
      ...
      await expect(
        freshRow.getByTestId("handwavy-history-batch-thrash-badge"),
      ).toHaveCount(0);
  ...
  // preview pattern.
  test("high-thrash batch auto-expands per-phrase rows; routine batches collapsed; manual collapse sticks", async ({
    page,
  }) => {
    const apiCtx = await newApiContext();
