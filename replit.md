...
- **Global Error Boundary (K1)**: Per-stage try/catch wrapping the analysis pipeline. Individual stages can fail without crashing the whole analysis. Sub-detectors within each stage are also individually wrapped via `safeDetector()` to prevent a single regex crash from zeroing all signals. Diagnostics (inputStats, stages, parseWarnings, totalDurationMs) included in API response.
