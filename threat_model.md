...
- **Residual risk.** A high-volume submitter can still skew distributional statistics that the calibration UI surfaces to reviewers, creating an indirect path to influence reviewer decisions. The per-source cap limits throughput but a determined attacker rotating IPs/user-agents can still contribute above the cap.
- **Mitigations implemented.** Per-visitor (HMAC-bucketed) submission caps enforced on corpus-eligible reports (configurable via `CORPUS_SUBMISSION_CAP`, default 20/day). Calibration changes that move weights past a configurable threshold (`CALIBRATION_DRIFT_THRESHOLD`, default 20%) require a second reviewer confirmation (`confirmDrift: true`). A rollback log snapshots calibration state before every mutation so poisoned changes can be reverted via `POST /feedback/calibration/rollback`.
